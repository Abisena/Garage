(() => {
    ...
    const HEADER_HTML = `<div class="gb-header" style="
    ...
    ">
        ...
        <span class="gb-c-code gb-hdr">KODE</span>
        <span class="gb-c-name gb-hdr">NAMA CABANG</span>
        <span class="gb-c-city gb-hdr">KOTA</span>
        <span class="gb-c-phone gb-hdr">TELEPON</span>
        <span class="gb-c-badge gb-hdr">STATUS</span>
        <span class="gb-c-ago gb-hdr"></span>
    </div>`;

    function card(doc) {
        const s = STATUS[String(cint(doc.is_active))] || STATUS["0"];

        const code  = doc.branch_code || doc.name;
        const name  = doc.branch_name || doc.name;
        const city  = doc.city || "";
        const phone = doc.phone || "";
        const ago   = doc.modified ? frappe.datetime.comment_when(doc.modified, true) : "";

        return `<div class="gb-card" style="
            display:flex !important;
            align-items:center;
            width:100%;
        ...
        ">
            ...
            <span class="gb-c-code">${esc(code)}</span>
            <span class="gb-c-name">${esc(name)}</span>
            <span class="gb-c-city">${esc(city)}</span>
            <span class="gb-c-phone">${esc(phone)}</span>
            <span class="gb-c-badge" style="background:${s.bg};color:${s.fg};">${s.label}</span>
            <span class="gb-c-ago">${ago}</span>
        </div>`;
    }

    function render(lv) {
        const $fl = lv.$result.closest(".frappe-list");
        if (!$fl.hasClass("gb-list")) $fl.addClass("gb-list");
        ...
        lv.$result.find(".list-row:not(.list-row-head)").each(function () {
            ...
            $card.on("click", function (e) {
                ...
                frappe.set_route("Form", "Garage Branch", doc.name);
            });
    ...
    frappe.listview_settings["Garage Branch"] = {
