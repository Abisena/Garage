...
def _item_to_spare_part_record(
    ...
    stock_map: Optional[Mapping[str, Mapping[str, float]]] = None,
    price_map: Optional[Mapping[str, float]] = None,
) -> Dict[str, Any]:
    part_code = cstr(
        item.get("part_code") or item.get("item_code") or item.get("name") or ""
    ).strip()
    stock_info = (stock_map or {}).get(part_code) or {}
    ...
    return {
        ...
        "category": item.get("category") or item.get("item_group"),
        ...
        "reorder_level": flt(item.get("reorder_level") or item.get("safety_stock") or 0),
        "warehouse_location": item.get("warehouse_location"),
        "managed_by": item.get("managed_by") or item.get("owner"),
        "status": (item.get("status") or "Active")
        if item.get("status")
        else ("Inactive" if cint(item.get("disabled")) else "Active"),
...
def _extract_payment_allocations(payload: Mapping[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not allocations:
        invoice = (
            payload.get("invoice")
            or payload.get("sales_invoice")
            or payload.get("reference_name")
        )
        if invoice:
            allocated_amount = flt(
                payload.get("allocated_amount")
                or payload.get("paid_amount")
                or payload.get("received_amount")
...
def sync_frontend_work_orders(work_orders: Optional[Any] = None) -> Dict[str, Any]:
    """Persist portal/localStorage work order state into Garage Service Orders
    ...
    # -------- Logging Summary --------
    summary = {"processed": len(work_orders), "updated": updated, "results": results}
    log_name = _log_integration_snapshot("Garage Frontend Sync", {"work_orders": work_orders}, summary)
...
def get_spare_part_stats() -> Dict[str, Any]:
    ...
    # Get all spare parts from Garage Spare Part master
