...
export function Registration({ currentUser }: RegistrationProps) {
  ...
  const vehicleModelsByBrand: { [key: string]: string[] } = {
  ...
  const getNextOrderNumber = (branch: string): string => {
  ...
  const handleInputComplete = (currentField: string, value: string) => {
    if (!value) return;
    ...
    if (currentIndex < fieldOrder.length - 1) {
      ...
      setTimeout(() => {
        const nextInput = document.querySelector(`[name="${nextField}"]`) as HTMLInputElement;
        if (nextInput) {
          nextInput.focus();
        }
      }, 100);
  ...
  const handleRegisterClick = () => {
    // Validate required fields (removed estimatedCost and estimatedDays)
    ...
    setTimeout(() => {
      const firstInput = document.querySelector('[name="plateNumber"]') as HTMLInputElement;
      if (firstInput) {
        firstInput.focus();
      }
    }, 100);
  ...
  const handleWorkOrderConfirm = (customerSig: string, advisorSig: string) => {
  ...
  const filteredRegistrations = currentUser.role === 'admin' && currentUser.branch === 'all'
    ? recentRegistrations  // Admin can see all
    : recentRegistrations.filter(reg => reg.branch === currentUser.branch);  // Branch user only sees their branch
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-slate-800">Customer Registration</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <h3 className="text-slate-800 mb-6">New Registration Form</h3>
              
              <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); handleRegisterClick(); }}>
                {/* Vehicle Information - NOW FIRST */}
                <div>
                  <h4 className="text-slate-700 mb-4 flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {/* 2. Chassis Number */}
                    <div>
                      <label className="block text-slate-700 mb-2">Chassis Number *</label>
                      <input
                        type="text"
                        name="chassisNumber"
                    ...
                    {/* 3. Engine Number */}
                    <div>
                      <label className="block text-slate-700 mb-2">Engine Number *</label>
                      <input
                        type="text"
                        name="engineNumber"
                    ...
                    {/* 6. Vehicle Type */}
                    <div>
                      <label className="block text-slate-700 mb-2">Vehicle Type *</label>
                      <select 
                        name="vehicleType"
                        className={`w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${
                    ...
                    {/* 7. Kilometer */}
                    <div>
                      <label className="block text-slate-700 mb-2">Kilometer *</label>
                      <input
                        type="text"
                        name="kilometer"
                    ...
                    {/* 8. Fuel */}
                    <div>
                      <label className="block text-slate-700 mb-2">Fuel *</label>
                      <select 
                        name="fuel"
                        className={`w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${
                    ...
                    {/* 9. Jenis Rakit */}
                    <div>
                      <label className="block text-slate-700 mb-2">Jenis Rakit *</label>
                      <select 
                        name="assemblyType"
                        className={`w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${
                ...
                {/* Service Type - Changed from textarea to select */}
                <div>
                ...
                {/* Customer Complaint - NEW FIELD */}
                <div>
                  <label className="block text-slate-700 mb-2">Customer Complaint *</label>
                  <textarea
                ...
                <div>
                  <label className="block text-slate-700 mb-2">Service Advisor Notes</label>
                  <textarea
                    name="advisorNotes"
                    rows={3}
            ...
            <div className="bg-white rounded-xl border border-slate-200 h-full flex flex-col">
              ...
              <div className="p-4 border-b border-slate-200">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-slate-800">Today's Registrations</h3>
                  <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full">{filteredRegistrations.length}</span>
                </div>
                {/* Search */}
                  ...
                  <table className="w-full">
                    <thead className="bg-slate-50 sticky top-0">
                      <tr className="border-b border-slate-200">
                        ...
                        <th className="px-3 py-2 text-left text-xs text-slate-600">Customer</th>
                        <th className="px-3 py-2 text-left text-xs text-slate-600">Vehicle</th>
                        <th className="px-3 py-2 text-left text-xs text-slate-600">Service</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredRegistrations
                        ...
                        >
                          <td className="px-3 py-2">
                            <span className="text-slate-900 text-sm">{reg.time}</span>
                          </td>
                          <td className="px-3 py-2">
                            <div>
                              <p className="text-slate-900 text-sm truncate">{reg.customerName}</p>
                              <p className="text-slate-500 text-xs">{reg.plateNumber}</p>
                            </div>
                          </td>
                          <td className="px-3 py-2">
                            <div>
