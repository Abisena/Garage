...
export function SpareParts() {
  ...
  const [formData, setFormData] = useState<Partial<MasterSparePart>>({
    partName: '',
    partNumber: '',
    compatibleModels: [],
    category: '',
  ...
  const handleEdit = (part: MasterSparePart) => {
  ...
  const handleSave = () => {
    if (isAddingNew) {
      ...
      setSpareParts(spareParts.map(part => 
        part.id === editingId ? { ...part, ...formData } as MasterSparePart : part
      ));
    }
