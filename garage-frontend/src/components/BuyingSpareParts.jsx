...
export function BuyingSparePartIntegrated({ currentUser }) {
  ...
  const getStatusBadge = (status, payoutNumber) => {
    ...
    return (
      <div className="flex items-center gap-2 flex-wrap justify-center">
        <span className={`px-2 py-0.5 ${badge.bg} ${badge.text} border ${badge.border} rounded-full flex items-center gap-1 text-xs whitespace-nowrap`}>
          <Icon className="w-3 h-3" />
          {badge.label}
        </span>
        {payoutNumber && (
          <span className="px-2 py-0.5 bg-red-100 text-red-700 border border-red-200 rounded-full flex items-center gap-1 text-xs whitespace-nowrap">
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {activeTab === 'po' ? (
          // Purchase Orders List - FIXED LAYOUT
          filteredPOs.length === 0 ? (
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              {/* ✅ FIXED: Add overflow-x-auto wrapper and optimize table layout */}
              <div className="overflow-x-auto">
                <table className="w-full min-w-max">
                  <thead className="bg-slate-100">
                    <tr>
                      <th className="px-3 py-2.5 text-left text-slate-700 text-sm font-medium whitespace-nowrap">PO Number</th>
                      <th className="px-3 py-2.5 text-left text-slate-700 text-sm font-medium whitespace-nowrap">Order Date</th>
                      <th className="px-3 py-2.5 text-left text-slate-700 text-sm font-medium whitespace-nowrap">Vendor</th>
                      <th className="px-3 py-2.5 text-left text-slate-700 text-sm font-medium whitespace-nowrap">Part Name</th>
                      <th className="px-3 py-2.5 text-left text-slate-700 text-sm font-medium whitespace-nowrap">Requested By</th>
                      <th className="px-3 py-2.5 text-left text-slate-700 text-sm font-medium whitespace-nowrap">Branch</th>
                      <th className="px-3 py-2.5 text-center text-slate-700 text-sm font-medium whitespace-nowrap">Qty</th>
                      <th className="px-3 py-2.5 text-right text-slate-700 text-sm font-medium whitespace-nowrap">Total Amount</th>
                      <th className="px-3 py-2.5 text-center text-slate-700 text-sm font-medium whitespace-nowrap">Status</th>
                      <th className="px-3 py-2.5 text-center text-slate-700 text-sm font-medium whitespace-nowrap">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredPOs.map((po) => (
                      <tr key={po.id} className="border-t border-slate-100 hover:bg-slate-50">
                        <td className="px-3 py-3">
                          <span className="text-blue-600 text-sm whitespace-nowrap">{po.poNumber}</span>
                        </td>
                        <td className="px-3 py-3 text-slate-700 text-sm whitespace-nowrap">{po.orderDate}</td>
                        <td className="px-3 py-3 text-slate-700 text-sm">
                          <div className="max-w-[140px] truncate" title={po.vendor}>{po.vendor}</div>
                        </td>
                        <td className="px-3 py-3 text-slate-700 text-sm">
                          <div className="max-w-[180px] truncate" title={po.items.map(item => item.partName).join(', ')}>
                            {po.items.length > 0 ? po.items[0].partName : '-'}
                            {po.items.length > 1 && ` +${po.items.length - 1}`}
                          </div>
                        </td>
                        <td className="px-3 py-3 text-slate-700 text-sm">
                          <div className="max-w-[110px] truncate" title={po.requestedBy}>{po.requestedBy}</div>
                        </td>
                        <td className="px-3 py-3">
                          <span className="inline-block px-2 py-0.5 rounded-full text-xs bg-slate-100 text-slate-700 whitespace-nowrap">
                            {po.branch}
                          </span>
                        </td>
                        <td className="px-3 py-3 text-center text-slate-700 text-sm whitespace-nowrap">
                          {po.items.reduce((sum, item) => sum + item.quantity, 0)}
                        </td>
                        <td className="px-3 py-3 text-right text-slate-900 text-sm whitespace-nowrap">{formatCurrency(po.totalAmount)}</td>
                          ...
                          <div className="flex gap-1.5 justify-center flex-nowrap">
                            {(po.status === 'PRINTED' || po.status === 'RECEIVED') && (
                              <Button
                                ...
                                className="border-blue-300 text-blue-600 hover:bg-blue-50 whitespace-nowrap text-xs px-2 py-1"
                              >
                            ...
                            {po.status === 'PRINTED' && !po.payoutNumber && (
                              <Button
                                ...
                                className="bg-emerald-500 hover:bg-emerald-600 text-white whitespace-nowrap text-xs px-2 py-1"
                              >
                            ...
                            {po.status === 'RECEIVED' && (
                              <span className="text-emerald-600 text-xs whitespace-nowrap">✓ Done</span>
                            )}
                            {po.payoutNumber && (
                              <span className="text-red-600 text-xs whitespace-nowrap flex items-center gap-1">
                                <AlertCircle className="w-3 h-3" />
                                Paid
                              </span>
                            )}
